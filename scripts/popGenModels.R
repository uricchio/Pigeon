...
read.table("~/projects/Pigeon/simData/sim.dx_T.0.5.d.0.05.noFlip.txt")->popSims
...
plA<-plA+geom_point(aes(V8,V1),size=1.3)+ylim(c(0.25,1))+scale_color_manual(values=wes_palette("Zissou1",6,type="continuous"),name=expression(italic(X[T])),guide='none')
...
plB<-plB+scale_color_manual(values=wes_palette("Zissou1",6,type="continuous"),name=expression(italic(T[c])))+geom_line(aes(V8,V5),lty=2,alpha=0.7)
plB<-plB +xlab("year")+scale_linetype(guide="none")+theme(text = element_text(size=20))+theme(axis.line=element_line(size=1.4))
  
upper<-plot_grid(plA,plB,labels=c("A","B"),ncol=2,rel_widths = c(1,1.3))

read.table("~/projects/Pigeon/simData/sim.dx_T.0.5.d.0.05.Flip.txt")->popSims
...
lower<-plot_grid(plA,plB,labels=c("C","D"),ncol=2,rel_widths = c(1,1.3))

plot_grid(upper,lower,ncol=1)

ggsave("~/projects/Pigeon/Figures/popGenSims.pdf",height=6.4,width=11)

# bigger rate of T inc

read.table("~/projects/Pigeon/simData/sim.dx_T.1.d.0.05.noFlip.txt")->popSims
...
plA<-ggplot(data=popSims,aes(V8,V3,col=as.factor(V7),linetype=as.factor(lt)))+geom_line(size=0.7,alpha=0.5)+ylab(expression(italic(f[aa])))+scale_x_continuous(trans='log10',breaks=c(1,10,100,1000),labels=c(1940,1950,2040,2940))
plA<-plA+theme_classic()
plA<-plA+geom_point(aes(V8,V1),size=1.3)+ylim(c(-0.1,1.1))+scale_color_manual(values=wes_palette("Zissou1",6,type="continuous"),name=expression(italic(X[T])),guide='none')
plA<-plA +xlab("year")+scale_linetype(guide="none")+theme(text = element_text(size=20))+theme(axis.line=element_line(size=1.4))
...
plStab<-ggplot(data=popSims,aes(V8,V10,col=as.factor(V7),linetype=as.factor(lt)))+geom_line(size=0.7,alpha=0.5)+ylab(expression(italic(dQ)))+scale_x_continuous(trans='log10',breaks=c(1,10,100,1000),labels=c(1940,1950,2040,2940))
plStab<-plStab+theme_classic()
plStab<-plStab+scale_color_manual(values=wes_palette("Zissou1",6,type="continuous"),name=expression(italic(X[T])),guide='none')
plStab<-plStab +xlab("year")+scale_linetype(guide="none")+theme(text = element_text(size=20))+theme(axis.line=element_line(size=1.4))+ylim(-0.003,0.01)

plStabA<-plStab

read.table("~/projects/Pigeon/simData/sim.dx_T.1.d.0.05.Flip.txt")->popSims
...
ggsave("~/projects/Pigeon/Figures/popGenSims.BiggerDeltaTc.pdf",height=6.4,width=11)
